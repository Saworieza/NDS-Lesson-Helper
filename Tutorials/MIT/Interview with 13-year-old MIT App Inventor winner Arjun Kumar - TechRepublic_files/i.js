...
var bcx_init = function(){
	...
	bouncex.cookie_domain = false;
				if(bouncex.calling_url.indexOf('techrepublic.com')>0){
				bouncex.cookie_domain = 'techrepublic.com';
			}
	...
	bouncex.wndsize = function(){
		...
		var init_vars = function() {
			...
			bouncex.varp = {max:0,c:0,timer:null,vn:[],suc:{},def:{}};
					...
					var n = 'is_logged_in';
					bouncex.vard[n] = { d: maked('false'), q: makeq('jQuery(\'[data-user-view=loggedIn]\').is(\':visible\')'),m:'all',p:false };
					initv(n);
					 
					bouncex.varp.vn.push(n);
					
					var n = 'already_engaged';
					bouncex.vard[n] = { d: maked('false'), q: makeq('jQuery(\'.modal-login.show\').length>0?true:false || jQuery(\'.modal-register\').length>0?true:false || jQuery(\'.modal-body\').is(\':visible\')>0?true:false || $(\'iframe[name*=\\\'google_ads_iframe_/8264/aw-techrepublic/\\\']\').is(\':visible\')'),m:'all',p:false };
					initv(n);
					 
					bouncex.varp.vn.push(n);
					
					var n = 'ever_logged_in';
					bouncex.vard[n] = { d: maked('false'), q: makeq('jQuery(\'[data-user-view=loggedIn]\').is(\':visible\') || bouncex.vars.ever_logged_in == true'),m:'all',p:true };
					initv(n);
					 
					bouncex.varp.vn.push(n);
					
					var n = 'not_in_iframe';
					bouncex.vard[n] = { d: maked('false'), q: makeq('window.self===window.top'),m:'all',p:false };
					initv(n);
					 
					bouncex.varp.vn.push(n);
					
					var n = 'flash_ad_active';
					bouncex.vard[n] = { d: maked('0'), q: makeq('(function(){ 	var obj = jQuery(\'object\'); 	var parObj = obj.parent().parent(); 	if (obj.length > 0) { 		return parObj[0].style.width == \'100%\'?true:false; 	} })();'),m:'all',p:false };
					initv(n);
					 
					bouncex.varp.vn.push(n);
					
					var n = 'new_ever_logged_in';
					bouncex.vard[n] = { d: maked('false'), q: makeq('$(\'span.header-member-logged-in\').is(\':visible\') || bouncex.vars.new_ever_logged_in'),m:'all',p:true };
					initv(n);
					 
					bouncex.varp.vn.push(n);
					
					var n = 'new_overlay_present';
					bouncex.vard[n] = { d: maked('false'), q: makeq('$(\'div[class*=modal]:visible\').length>0'),m:'all',p:false };
					initv(n);
					 
					bouncex.varp.vn.push(n);
								bouncex.varp.max = 30;
			bouncex.varp.timer = setInterval(checkPolling,1000);
		...
		bouncex.make_include_string = function(type){
			...
			var bcx_str = window.location.protocol + '//bounceexchange.com/bounce/'+type+'.js?tojQ='+tojQ+'&tzo='+tzo+'&is_preview=false&website_id=786&resolution='+resolution+'&referrer='+bouncex.referrer+'&calling_url='+bouncex.calling_url+'&visit_cookie='+escape(visit_cookie)+'&cookie='+escape(cookie)+bcx_vars.replace(/\+/g,'%2B')+bcx_persist;
			return bcx_str;
		}
...
function setBounceVisitCookie(){
	var c_name = 'bounceClientVisit786v';
	var cookie = bouncex.visit_cookie;
	var exdate = new Date();
	exdate.setTime(exdate.getTime() + 30*60*1000);
	var c_value=((bouncex.stringify(bouncex.visit_cookie)) + "; expires=" + exdate.toUTCString());
...
function getBounceVisitCookie(name){
	...
	function setBounceCookie(visit){
		var c_name = 'bounceClientVisit786';
					var exdate = new Date();
			exdate.setDate(exdate.getDate() + 365);
			var c_value=((bouncex.stringify(bouncex.cookie)) + "; expires=" + exdate.toUTCString());
			if(bouncex.cookie_domain){
				document.cookie =  (c_name + "=" + c_value + ";path=/;domain=."+bouncex.cookie_domain+";");
			}else{
				document.cookie =  (c_name + "=" + c_value + ";path=/;");
